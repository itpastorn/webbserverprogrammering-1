 ...
 * svarsmejladress och/eller namn på den som skickar meddelandet,
 *
 ...
 *
 * Det finns upp till 10 variabler att använda.
 * 
 * Båda mallarna kan använda följande variabler, som skrivs inom klammerparenteser, som nedan:
 * 
 * - {$uname}    - "User name"       = Namn på den som fyller i formuläret, 2-100 tecken
 * - {$umail}    - "User mail"       = Adress till den som fyller i formuläret, en korrekt mejladress
 * - {$msubject} - "Message subject" = Ärendet 5-150 tecken - Alltid obligatorisk!
 * - {$mmessage} - "Message message" = Meddelandet 25-2500 tecken - Alltid obligatorisk!
 * 
 * Genom att sätta dessa som attributvärden för <input value>, eller som värde för textarea,
 * så slipper användaren upprepa korrekt inskickad data, om något fält innehåller fel.
 * 
 * Till dessa hör ett matchande meddelande, för formulärmallen:
 * - {$uname_extra}
 * - {$umail_extra}
 * - {$msubject_extra}
 * - {$mmessage_extra}
 * 
 * Dessa meddelanden kommer sättas till konfigurationvariabeln $required_msg för alla obligatoriska värden
 * vid en initial formulärvisning. Därtill ska de användas som felmmeddelanden, om servern hittar
 * några felaktiga uppgifter
 * 
 * Variablen $required_msg i sig kan inte användas direkt i mallen.
 * 
 * Dessutom MÅSTE följande fält finnas i formuläret:
 ...
 * {$smtpsafe_message} - Själva meddelandetexten, SMTP-säkrad.
 *
...
$random_string = $_SESSION['prevent_multiple_submits'];
